...
public class DList extends List implements Iterable<DListNode> {
     ...
     **/
    public DList() {
        this.head = new DListNode(null, null, null, null);
        this.head.next = head;
        this.head.prev = head;
        this.size = 0;
    }
    ...
    public void insertFront(Object item) {
        DListNode toInsert = new DListNode(item, this, this.head, this.head.next);
        this.head.next.prev = toInsert;
    ...
    public void insertBack(Object item) {
        DListNode toInsert = new DListNode(item, this,this.head.prev, this.head);
        this.head.prev.next = toInsert;
    ...
    public boolean hasItem(Object item){
        for (DListNode node: this){
            try {
                if (node.item().equals(item)){return true;}
            } catch (InvalidNodeException e) {
                System.out.println("Invalid node when checking if DList contain a certain item.");
            }
        }
        return false;
    }
    ...
    public DList clone(){
        DList newPath = new DList();
        try {
            for (DListNode node: this){
                newPath.insertBack(node.item());
            }
        } catch (InvalidNodeException e){
            System.out.println("Invalid node when cloning DList.");
        }
